...
void *fake_mmap(size_t size) {
  ...
  // region which avoids work when accessing the pages later. However it causes
  // long pauses when mmapping the files. Only supported on Linux.
  void *pointer = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (pointer == MAP_FAILED) {
    ARROW_LOG(ERROR) << "mmap failed with error: " << std::strerror(errno);
    if (errno == ENOMEM && plasma_config->hugepages_enabled) {
      ARROW_LOG(ERROR) << "  (this probably means you have to increase "
