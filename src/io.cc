...
Status WriteMessage(int fd, MessageType type, int64_t length, uint8_t *bytes) {
  ...
  RETURN_NOT_OK(
      WriteBytes(fd, reinterpret_cast<uint8_t *>(&length_le), sizeof(length)));
...
Status ReadMessage(int fd, MessageType *type, std::vector<uint8_t> *buffer) {
  ...
  *type = static_cast<MessageType>(
  ...
  size_t length =
      static_cast<size_t>(arrow::bit_util::FromLittleEndian(length_temp));
...
std::unique_ptr<uint8_t[]> ReadMessageAsync(int sock) {
  int64_t size;
  Status s =
      ReadBytes(sock, reinterpret_cast<uint8_t *>(&size), sizeof(int64_t));
  if (!s.ok()) {
