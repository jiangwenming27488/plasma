...
void LRUCache::AdjustCapacity(int64_t delta) {
  ARROW_LOG(INFO) << "adjusting global lru capacity from " << Capacity()
                  << " to " << (Capacity() + delta) << " (max "
                  << OriginalCapacity() << ")";
  capacity_ += delta;
  ARROW_CHECK(used_capacity_ >= 0) << DebugString();
}
...
int64_t LRUCache::RemainingCapacity() const {
...
std::string LRUCache::DebugString() const {
  ...
  result << "\n(" << name_ << ") used: "
         << 100. * (1. - (RemainingCapacity() / (double)OriginalCapacity()))
         << "%";
...
int64_t
LRUCache::ChooseObjectsToEvict(int64_t num_bytes_required,
...
EvictionPolicy::EvictionPolicy(PlasmaStoreInfo *store_info, int64_t max_size)
    : pinned_memory_bytes_(0), store_info_(store_info),
      cache_("global lru", max_size) {}

int64_t
EvictionPolicy::ChooseObjectsToEvict(int64_t num_bytes_required,
                                     std::vector<ObjectID> *objects_to_evict) {
...
bool EvictionPolicy::SetClientQuota(Client *client,
                                    int64_t output_memory_quota) {
  return false;
}

bool EvictionPolicy::EnforcePerClientQuota(
    Client *client, int64_t size, bool is_create,
    std::vector<ObjectID> *objects_to_evict) {
  return true;
}
...
bool EvictionPolicy::RequireSpace(int64_t size,
                                  std::vector<ObjectID> *objects_to_evict) {
  // Check if there is enough space to create the object.
  int64_t required_space = PlasmaAllocator::Allocated() + size -
