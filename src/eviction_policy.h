...
public:
  LRUCache(const std::string &name, int64_t size)
      : name_(name), original_capacity_(size), capacity_(size),
        used_capacity_(0), num_evictions_total_(0), bytes_evicted_total_(0) {}
...
public:
  ...
  /// \param is_create Whether we are creating a new object (vs reading an
  /// object).
  virtual void ObjectCreated(const ObjectID &object_id, Client *client,
                             bool is_create);
  ...
  ///        called once per client. This is effectively the equivalent of
  ///        giving the client its own LRU cache instance. The memory for this
  ///        is taken out of the capacity of the global LRU cache for the client
  ///        lifetime.
  ///
  ...
  /// Determine what objects need to be evicted to enforce the given client's
  /// quota.
  ///
  /// \param client The pointer to the client creating the object.
  /// \param size The size of the object to create.
  /// \param is_create Whether we are creating a new object (vs reading an
  /// object). \param objects_to_evict The object IDs that were chosen for
  /// eviction will
  ///        be stored into this vector.
  ///
  /// \return True if enough space could be freed and false otherwise.
  virtual bool EnforcePerClientQuota(Client *client, int64_t size,
                                     bool is_create,
                                     std::vector<ObjectID> *objects_to_evict);
  ...
  virtual bool RequireSpace(int64_t size,
                            std::vector<ObjectID> *objects_to_evict);
