...
GetRequest::GetRequest(Client *client, const std::vector<ObjectID> &object_ids)
    : client(client), timer(-1),
      object_ids(object_ids.begin(), object_ids.end()),
      objects(object_ids.size()), num_satisfied(0) {
...
PlasmaStore::PlasmaStore(EventLoop *loop, std::string directory,
...
const PlasmaStoreInfo *PlasmaStore::GetPlasmaStoreInfo() {
  return &store_info_;
}
...
uint8_t *PlasmaStore::AllocateMemory(size_t size, bool evict_if_full, int *fd,
  ...
  if (evict_if_full) {
    ...
    bool quota_ok = eviction_policy_.EnforcePerClientQuota(
  ...
  while (true) {
    ...
    pointer = reinterpret_cast<uint8_t *>(
        PlasmaAllocator::Memalign(kBlockSize, size));
...
PlasmaStore::GetCudaContext(int device_num) {
...
Status PlasmaStore::AllocateCudaMemory(
  ...
  ARROW_ASSIGN_OR_RAISE(auto cuda_buffer,
                        context->Allocate(static_cast<int64_t>(size)));
...
Status PlasmaStore::FreeCudaMemory(int device_num, int64_t size,
                                   uint8_t *pointer) {
  ARROW_ASSIGN_OR_RAISE(auto context, GetCudaContext(device_num));
...
PlasmaError PlasmaStore::CreateObject(const ObjectID &object_id,
  ...
  if (device_num == 0) {
    pointer = AllocateMemory(total_size, evict_if_full, &fd, &map_size, &offset,
                             client, true);
    if (!pointer) {
      ARROW_LOG(ERROR) << "Not enough memory to create the object "
                       << object_id.hex() << ", data_size=" << data_size
  ...
  entry = store_info_.objects.emplace(object_id, std::move(ptr))
              .first->second.get();
...
void PlasmaStore::RemoveGetRequest(GetRequest *get_request) {
  ...
  for (ObjectID &object_id : get_request->object_ids) {
    ...
    if (object_request_iter != object_get_requests_.end()) {
      ...
      auto it =
          std::find(get_requests.begin(), get_requests.end(), get_request);
...
void PlasmaStore::RemoveGetRequestsForClient(Client *client) {
  ...
  // It shouldn't be possible for a given client to be in the middle of multiple
  // get requests.
...
void PlasmaStore::ReturnFromGet(GetRequest *get_req) {
  ...
  if (s.ok()) {
    ...
    for (int store_fd : store_fds) {
      ...
      if (get_req->client->used_fds.find(store_fd) ==
          get_req->client->used_fds.end()) {
...
void PlasmaStore::ProcessGetRequest(Client *client,
  ...
  for (auto object_id : object_ids) {
    ...
    if (entry && entry->state == ObjectState::PLASMA_SEALED) {
      ...
      entry->pointer = AllocateMemory(
          entry->data_size + entry->metadata_size, /*evict=*/true, &entry->fd,
  ...
  if (!evicted_ids.empty()) {
    ...
    for (size_t i = 0; i < evicted_ids.size(); ++i) {
      ...
      buffers.emplace_back(new arrow::MutableBuffer(
          evicted_entries[i]->pointer, evicted_entries[i]->data_size));
    }
    if (external_store_->Get(evicted_ids, buffers).ok()) {
      ...
      // We tried to get the objects from the external store, but could not get
      // them. Set the state of these objects back to PLASMA_EVICTED so some
      // other request can try again.
      for (size_t i = 0; i < evicted_ids.size(); ++i) {
  ...
  if (get_req->num_satisfied == get_req->num_objects_to_wait_for ||
    ...
    get_req->timer =
        loop_->AddTimer(timeout_ms, [this, get_req](int64_t timer_id) {
          ReturnFromGet(get_req);
...
#ifdef PLASMA_CUDA
    ARROW_CHECK_OK(
        FreeCudaMemory(object->device_num, buff_size, object->pointer));
#endif
  }
...
PlasmaStore::NotificationMap::iterator
PlasmaStore::SendNotifications(PlasmaStore::NotificationMap::iterator it) {
  ...
  for (size_t i = 0; i < notifications.size(); ++i) {
    ...
    ssize_t nbytes =
        send(client_fd, notification.get(), sizeof(int64_t) + size, 0);
    if (nbytes >= 0) {
      ...
      // is added twice.
      loop_->AddFileEvent(
          client_fd, kEventLoopWrite, [this, client_fd](int events) {
...
void PlasmaStore::SubscribeToUpdates(Client *client) {
  ...
  for (const auto &entry : store_info_.objects) {
    if (entry.second->state == ObjectState::PLASMA_SEALED) {
      ...
      info.digest = std::string(
          reinterpret_cast<char *>(&entry.second->digest[0]), kDigestSize);
...
Status PlasmaStore::ProcessMessage(Client *client) {
  ...
  case fb::MessageType::PlasmaCreateRequest: {
    ...
    PlasmaError error_code =
        CreateObject(object_id, evict_if_full, data_size, metadata_size,
  ...
  case fb::MessageType::PlasmaCreateAndSealRequest: {
    ...
    int device_num = 0;
    PlasmaError error_code =
        CreateObject(object_id, evict_if_full, data.size(), metadata.size(),
                     device_num, client, &object);

    // If the object was successfully created, fill out the object data and seal
    // it.
  ...
  case fb::MessageType::PlasmaCreateAndSealBatchRequest: {
    ...
    RETURN_NOT_OK(ReadCreateAndSealBatchRequest(input, input_size, &object_ids,
                                                &evict_if_full, &data,
                                                &metadata, &digests));
    ...
    for (i = 0; i < object_ids.size(); i++) {
      error_code =
          CreateObject(object_ids[i], evict_if_full, data[i].size(),
    ...
    if (error_code == PlasmaError::OK) {
      ...
      for (i = 0; i < object_ids.size(); i++) {
        auto entry = GetObjectTableEntry(&store_info_, object_ids[i]);
        ARROW_CHECK(RemoveFromClientObjectIds(object_ids[i], entry, client) ==
                    1);
      }
    } else {
  ...
  case fb::MessageType::PlasmaAbortRequest: {
    ...
    ARROW_CHECK(AbortObject(object_id, client) == 1)
        << "To abort an object, the only "
  ...
  case fb::MessageType::PlasmaDeleteRequest: {
    ...
    HANDLE_SIGPIPE(SendDeleteReply(client->fd, object_ids, error_codes),
                   client->fd);
  } break;
  ...
  case fb::MessageType::PlasmaConnectRequest: {
    HANDLE_SIGPIPE(
        SendConnectReply(client->fd, PlasmaAllocator::GetFootprintLimit()),
        client->fd);
  } break;
  ...
  case fb::MessageType::PlasmaSetOptionsRequest: {
    ...
    RETURN_NOT_OK(ReadSetOptionsRequest(input, input_size, &client_name,
                                        &output_memory_quota));
    client->name = client_name;
    bool success = eviction_policy_.SetClientQuota(client, output_memory_quota);
    HANDLE_SIGPIPE(
        SendSetOptionsReply(client->fd, success ? PlasmaError::OK
                                                : PlasmaError::OutOfMemory),
        client->fd);
  } break;
  case fb::MessageType::PlasmaGetDebugStringRequest: {
    HANDLE_SIGPIPE(
        SendGetDebugStringReply(client->fd, eviction_policy_.DebugString()),
        client->fd);
  } break;
  default:
...
public:
  ...
  void Start(char *socket_name, std::string directory, bool hugepages_enabled,
    ...
    store_.reset(new PlasmaStore(loop_.get(), directory, hugepages_enabled,
                                 socket_name, external_store));
    plasma_config = store_->GetPlasmaStoreInfo();

    // We are using a single memory-mapped file by mallocing and freeing a
    // single large amount of space up front. According to the documentation,
...
// Command-line flags.
DEFINE_string(d, SHM_DEFAULT_PATH,
...
DEFINE_string(m, "",
              "amount of memory in bytes to use for Plasma store, required");
...
int main(int argc, char *argv[]) {
  ...
  if (!FLAGS_m.empty()) {
    char extra;
    int scanned =
        sscanf(FLAGS_m.c_str(), "%" SCNd64 "%c", &system_memory, &extra);
    if (scanned != 1) {
    ...
    plasma::PlasmaAllocator::SetFootprintLimit(
        static_cast<size_t>(system_memory));
  ...
  if (!hugepages_enabled) {
    ...
    if (system_memory > shm_mem_avail) {
      ARROW_LOG(WARNING)
          << "System memory request exceeds memory available in "
          << plasma_directory << ". The request is for " << system_memory
  ...
  if (!external_store_endpoint.empty()) {
    std::string name;
    ARROW_CHECK_OK(plasma::ExternalStores::ExtractStoreName(
        external_store_endpoint, &name));
