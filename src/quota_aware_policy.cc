...
QuotaAwarePolicy::QuotaAwarePolicy(PlasmaStoreInfo *store_info,
                                   int64_t max_size)
...
bool QuotaAwarePolicy::SetClientQuota(Client *client,
  ...
  if (cache_.Capacity() - output_memory_quota <
    ...
    ARROW_LOG(WARNING) << "Not enough memory to set client quota: "
                       << DebugString();
    return false;
  }
...
bool QuotaAwarePolicy::EnforcePerClientQuota(
  ...
  if (space_to_free > 0) {
    ...
    for (ObjectID &object_id : candidates) {
      if (shared_for_read_.count(object_id)) {
        // Pinned so we can't evict it, so demote the object to global LRU
        // instead. We an do this by simply removing it from all data
        // structures, so that the next EndObjectAccess() will add it back to
        // global LRU.
