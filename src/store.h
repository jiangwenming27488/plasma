...
public:
  ...
  /// Delete a specific object by object_id that have been created in the hash
  /// table.
  ///
  ...
  /// Seal a vector of objects. The objects are now immutable and can be
  /// accessed with get.
  ///
  /// \param object_ids The vector of Object IDs of the objects to be sealed.
  /// \param digests The vector of digests of the objects. This is used to tell
  /// if two objects with the same object ID are the same.
...
private:
  ...
  uint8_t *AllocateMemory(size_t size, bool evict_if_full, int *fd,
                          int64_t *map_size, ptrdiff_t *offset, Client *client,
                          bool is_create);
#ifdef PLASMA_CUDA
  arrow::Result<std::shared_ptr<arrow::cuda::CudaContext>>
  GetCudaContext(int device_num);
