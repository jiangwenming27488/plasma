...
// Forward declaration outside the namespace, which is defined in
// plasma_generated.h.
...
Status WriteMessage(int fd, flatbuf::MessageType type, int64_t length,
                    uint8_t *bytes);

Status ReadBytes(int fd, uint8_t *cursor, size_t length);

Status ReadMessage(int fd, flatbuf::MessageType *type,
