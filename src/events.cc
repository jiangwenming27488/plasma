...
int EventLoop::TimerEventCallback(aeEventLoop *loop, TimerID timer_id,
                                  void *context) {
...
bool EventLoop::AddFileEvent(int fd, int events, const FileCallback &callback) {
  ...
  if (err == AE_ERR && errno == ERANGE) {
    ...
    err = aeCreateFileEvent(loop_, fd, events, EventLoop::FileEventCallback,
                            context);
  }
...
int64_t EventLoop::AddTimer(int64_t timeout, const TimerCallback &callback) {
  ...
  int64_t timer_id = aeCreateTimeEvent(
      loop_, timeout, EventLoop::TimerEventCallback, context, NULL);
