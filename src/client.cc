...
MakeBufferFromGpuProcessHandle(GpuProcessHandle *handle) {
  return std::make_shared<CudaBuffer>(
      handle->ptr->address(), handle->ptr->size(), handle->ptr->context());
}
...
public:
  ...
  PlasmaBuffer(std::shared_ptr<PlasmaClient::Impl> client,
               const ObjectID &object_id, const std::shared_ptr<Buffer> &buffer)
...
class PlasmaClient::Impl
    : public std::enable_shared_from_this<PlasmaClient::Impl> {
public:
  Impl();
  ~Impl();
  ...
  Status SetClientOptions(const std::string &client_name,
  ...
  Status Get(const ObjectID *object_ids, int64_t num_objects,
  ...
  Status DecodeNotifications(const uint8_t *buffer,
...
private:
  ...
  bool ComputeObjectHashParallel(XXH64_state_t *hash_state,
                                 const unsigned char *data, int64_t nbytes);
...
uint8_t *PlasmaClient::Impl::LookupOrMmap(int fd, int store_fd_val,
                                          int64_t map_size) {
  ...
  if (entry != mmap_table_.end()) {
    ...
    mmap_table_[store_fd_val] =
        std::make_unique<ClientMmapTableEntry>(fd, map_size);
...
#ifdef PLASMA_CUDA
arrow::Result<std::shared_ptr<CudaContext>>
PlasmaClient::Impl::GetCudaContext(int device_number) {
...
Status PlasmaClient::Impl::Create(const ObjectID &object_id, int64_t data_size,
  ...
  ARROW_LOG(DEBUG) << "called plasma_create on conn " << store_conn_
  ...
  RETURN_NOT_OK(
      PlasmaReceive(store_conn_, MessageType::PlasmaCreateReply, &buffer));
  ObjectID id;
  PlasmaObject object;
  int store_fd;
  int64_t mmap_size;
  ...
  if (device_num == 0) {
    ...
    *data = std::make_shared<PlasmaMutableBuffer>(
    ...
    // the metadata here. The metadata will be written along with the data
    // streamed from the transfer.
    if (metadata != NULL) {
    ...
    ARROW_ASSIGN_OR_RAISE(handle->ptr,
...
Status PlasmaClient::Impl::CreateAndSeal(const ObjectID &object_id,
  ...
  RETURN_NOT_OK(SendCreateAndSealRequest(store_conn_, object_id, evict_if_full,
                                         data, metadata, digest));
  std::vector<uint8_t> buffer;
  RETURN_NOT_OK(PlasmaReceive(store_conn_,
                              MessageType::PlasmaCreateAndSealReply, &buffer));
  RETURN_NOT_OK(ReadCreateAndSealReply(buffer.data(), buffer.size()));
  return Status::OK();
}

Status
PlasmaClient::Impl::CreateAndSealBatch(const std::vector<ObjectID> &object_ids,
                                       const std::vector<std::string> &data,
                                       const std::vector<std::string> &metadata,
                                       bool evict_if_full) {
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
  ...
  RETURN_NOT_OK(SendCreateAndSealBatchRequest(
      store_conn_, object_ids, evict_if_full, data, metadata, digests));
  std::vector<uint8_t> buffer;
  RETURN_NOT_OK(PlasmaReceive(
      store_conn_, MessageType::PlasmaCreateAndSealBatchReply, &buffer));
...
Status PlasmaClient::Impl::GetBuffers(
  ...
  for (int64_t i = 0; i < num_objects; ++i) {
    ...
    if (object_entry == objects_in_use_.end()) {
      ...
      ARROW_LOG(WARNING) << "Attempting to get an object that this client "
                            "created but hasn't sealed.";
      all_present = false;
    } else {
  ...
  RETURN_NOT_OK(
      SendGetRequest(store_conn_, &object_ids[0], num_objects, timeout_ms));
  std::vector<uint8_t> buffer;
  RETURN_NOT_OK(
    ...
    if (object->data_size != -1) {
      ...
      if (object->device_num == 0) {
        ...
        if (iter == gpu_object_map.end()) {
          ARROW_ASSIGN_OR_RAISE(auto context,
                                GetCudaContext(object->device_num));
...
Status PlasmaClient::Impl::Get(const std::vector<ObjectID> &object_ids,
  ...
  const auto wrap_buffer = [&](const ObjectID &object_id,
    ...
    return std::make_shared<PlasmaBuffer>(shared_from_this(), object_id,
                                          buffer);
  };
...
Status PlasmaClient::Impl::Contains(const ObjectID &object_id,
                                    bool *has_object) {
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
  ...
  if (objects_in_use_.count(object_id) > 0) {
    *has_object = 1;
    ...
    RETURN_NOT_OK(ReadContainsReply(buffer.data(), buffer.size(), &object_id2,
                                    has_object));
  }
  return Status::OK();
}
...
Status PlasmaClient::Impl::List(ObjectTable *objects) {
  ...
  std::vector<uint8_t> buffer;
  RETURN_NOT_OK(
      PlasmaReceive(store_conn_, MessageType::PlasmaListReply, &buffer));
...
bool PlasmaClient::Impl::ComputeObjectHashParallel(XXH64_state_t *hash_state,
  ...
  for (int i = 0; i < num_threads; i++) {
    futures.push_back(*pool->Submit(ComputeBlockHash,
                                    reinterpret_cast<uint8_t *>(data_address) +
                                        i * chunk_size,
...
uint64_t PlasmaClient::Impl::ComputeObjectHashCPU(const uint8_t *data,
                                                  int64_t data_size,
                                                  const uint8_t *metadata,
                                                  int64_t metadata_size) {
  DCHECK(metadata);
...
Status PlasmaClient::Impl::Abort(const ObjectID &object_id) {
  ...
  if (object_entry->second->count > 1) {
    return Status::Invalid(
        "Plasma client cannot have a reference to the buffer.");
  }
...
Status PlasmaClient::Impl::Delete(const std::vector<ObjectID> &object_ids) {
  ...
  if (not_in_use_ids.size() > 0) {
    ...
    RETURN_NOT_OK(ReadDeleteReply(buffer.data(), buffer.size(), &not_in_use_ids,
                                  &error_codes));
  }
  return Status::OK();
}

Status PlasmaClient::Impl::Evict(int64_t num_bytes,
...
Status PlasmaClient::Impl::GetNotification(int fd, ObjectID *object_id,
  ...
  if (pending_notification_.empty()) {
    ...
    std::vector<int64_t> data_sizes;
    std::vector<int64_t> metadata_sizes;
    RETURN_NOT_OK(DecodeNotifications(message.get(), &object_ids, &data_sizes,
                                      &metadata_sizes));
    for (size_t i = 0; i < object_ids.size(); ++i) {
      pending_notification_.emplace_back(object_ids[i], data_sizes[i],
                                         metadata_sizes[i]);
    }
  }
...
Status PlasmaClient::Impl::DecodeNotifications(
    const uint8_t *buffer, std::vector<ObjectID> *object_ids,
    std::vector<int64_t> *data_sizes, std::vector<int64_t> *metadata_sizes) {
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
  auto object_info = flatbuffers::GetRoot<fb::PlasmaNotification>(buffer);

  for (flatbuffers::uoffset_t i = 0; i < object_info->object_info()->size();
       ++i) {
...
Status PlasmaClient::Impl::Connect(const std::string &store_socket_name,
  ...
  if (release_delay != 0) {
    ARROW_LOG(WARNING)
        << "The release_delay parameter in PlasmaClient::Connect "
        << "is deprecated";
  }
  ...
  std::vector<uint8_t> buffer;
  RETURN_NOT_OK(
      PlasmaReceive(store_conn_, MessageType::PlasmaConnectReply, &buffer));
  RETURN_NOT_OK(
      ReadConnectReply(buffer.data(), buffer.size(), &store_capacity_));
  return Status::OK();
}
...
Status PlasmaClient::Impl::SetClientOptions(const std::string &client_name,
  ...
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
  RETURN_NOT_OK(
      SendSetOptionsRequest(store_conn_, client_name, output_memory_quota));
  std::vector<uint8_t> buffer;
  RETURN_NOT_OK(
...
std::string PlasmaClient::Impl::DebugString() {
  ...
  if (!ReadGetDebugStringReply(buffer.data(), buffer.size(), &debug_string)
           .ok()) {
...
Status PlasmaClient::CreateAndSeal(const ObjectID &object_id,
                                   const std::string &data,
                                   const std::string &metadata,
                                   bool evict_if_full) {
...
Status
PlasmaClient::CreateAndSealBatch(const std::vector<ObjectID> &object_ids,
                                 const std::vector<std::string> &data,
                                 const std::vector<std::string> &metadata,
                                 bool evict_if_full) {
...
Status PlasmaClient::GetNotification(int fd, ObjectID *object_id,
                                     int64_t *data_size,
                                     int64_t *metadata_size) {
...
Status PlasmaClient::DecodeNotifications(const uint8_t *buffer,
                                         ...
                                         std::vector<int64_t> *data_sizes,
                                         std::vector<int64_t> *metadata_sizes) {
  return impl_->DecodeNotifications(buffer, object_ids, data_sizes,
                                    metadata_sizes);
}
