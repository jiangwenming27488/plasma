...
struct Client {
  ...
  /// valid if client subscribes to plasma store. -1 indicates invalid.
  int notification_fd;
...
/// check the success of messages sent to plasma clients. We print a warning
/// instead of failing because the plasma clients are allowed to die. This is
/// used to handle situations where the store writes to a client file
/// descriptor, and the client may already have disconnected. If we have
/// processed the disconnection and closed the file descriptor, we should get a
/// BAD FILE DESCRIPTOR error. If we have not, then we should get a SIGPIPE. If
/// we write to a TCP socket that isn't connected yet, then we should get an
/// ECONNRESET.
///
...
CreateObjectInfoBuffer(flatbuf::ObjectInfoT *object_info);

std::unique_ptr<uint8_t[]>
CreatePlasmaNotificationBuffer(std::vector<flatbuf::ObjectInfoT> &object_info);
