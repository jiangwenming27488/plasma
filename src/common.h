...
ARROW_EXPORT arrow::Status MakePlasmaError(PlasmaErrorCode code,
                                           std::string message);
...
/// Return true iff the status indicates the Plasma store reached its capacity
/// limit.
...
static_assert(std::is_trivial_v<UniqueID> &&
                  std::is_standard_layout_v<UniqueID>,
...
typedef std::unordered_map<ObjectID, std::unique_ptr<ObjectTableEntry>>
    ObjectTable;
...
namespace std {
template <> struct hash<::plasma::UniqueID> {
